...
function []=DensTradPlots(var1, var2, FLASH, xoffset, y1unit, y2unit, plottime)
figure;
...
xlabel('Position after target front surface (cm)');
ylabel(AX(1),'Density (g/cm^{3})');
...
%hold( ax2,'on');
ylabel(AX(2),'Radiation Temperature (eV)');
title(['Radial Electron Density and Electron Temperature at ',num2str(plottime(1)*0.05,'%.2f'),' ns']);
hold(AX(1),'on');
hold(AX(2),'on');
